...
func TestIncTrieInsertAndHash(t *testing.T) {
...
func TestIncTrieSizeBug(t *testing.T) {
	st := NewIncTrie()
	defer st.Done()
...
func TestIncTrieEmptyBug(t *testing.T) {
	st := NewIncTrie()
	defer st.Done()
	nt, _ := New(common.Hash{}, NewDatabase(memorydb.New()))
...
func TestIncTrieValLength56(t *testing.T) {
	st := NewIncTrie()
	defer st.Done()
	nt, _ := New(common.Hash{}, NewDatabase(memorydb.New()))
...
func TestIncTrieUpdateSmallNodes(t *testing.T) {
	st := NewIncTrie()
	defer st.Done()
	nt, _ := New(common.Hash{}, NewDatabase(memorydb.New()))
	kvs := []struct {
	...
	fmt.Printf("sss\n")
	if got, want := st.Hash(), nt.Hash(); got != want {
		t.Fatalf("error, got %x want %x", got, want)
	}
}
...
//
// The Inctrie passes this test (which stacktrie fails).
func TestIncTrieUpdateVariableKeys(t *testing.T) {
	st := NewIncTrie()
	defer st.Done()
	nt, _ := New(common.Hash{}, NewDatabase(memorydb.New()))
	kvs := []struct {
...
func TestIncTrieNotModifyValues(t *testing.T) {
	st := NewIncTrie()
	defer st.Done()
...
func BenchmarkInctrie10K(b *testing.B) {
	count := 10000
	st := NewStackTrie(nil)
	it := NewIncTrie()
	...
	key := make([]byte, 32)
	val := make([]byte, 32)
	var stHash common.Hash
	var itHash common.Hash
	b.Run("stacktrie", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			st.Reset()
			for ii := 0; ii < count; ii++ {
				binary.BigEndian.PutUint64(key, uint64(ii))
	...
	b.Run("inctrie", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			it.root = nil
			for ii := 0; ii < count; ii++ {
				binary.BigEndian.PutUint64(key, uint64(ii))
				binary.BigEndian.PutUint64(val, uint64(ii))
				it.TryUpdate(key, common.CopyBytes(val))
			}
			itHash = it.Hash()
		}
	})
	if got, want := itHash, stHash; got != want {
