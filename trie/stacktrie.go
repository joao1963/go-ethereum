...
func (t *StackTrie) Update(key, value []byte) error {
	...
	t.insert(t.root, k, value, nil, newLeaf)
	return nil
}
...
// the trie.
func (t *StackTrie) insert(st *stNode, key, value []byte, path []byte, leafCtor func([]byte, []byte) *stNode) {
	switch st.typ {
	case branchNode: /* Branch */
		...
		if st.children[idx] == nil {
			...
			t.insert(st.children[idx], key[1:], value, append(path, key[0]), leafCtor)
		}
	...
	case extNode: /* Ext */
		...
		if diffidx == len(st.key) {
			...
			// the child node.
			t.insert(st.children[0], key[diffidx:], value, append(path, key[:diffidx]...), leafCtor)
			return
		}
